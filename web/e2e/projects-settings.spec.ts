...
async function mockCommon(page: import("@playwright/test").Page) {
  ...
  await page.route(/\/api\/folders(\?.*)?$/, (r) =>
    r.fulfill({ json: { folders: [{ cwd: "/home/u/cayoo", label: "/home/u/cayoo" }] } }),
  );
...
test.describe("Settings → Projects manager (#361/#448)", () => {
  ...
  test("lists a project with its count, default folder, and folder chip", async ({ page }) => {
    await page.route(/\/api\/projects(\?.*)?$/, (r) => r.fulfill({ json: { projects: [CAYOO] } }));
  ...
  test("star: setting the default project persists default_project_id (#615)", async ({ page }) => {
    ...
    const star = page.getByRole("button", { name: "Make Cayoo the default project" });
    ...
    await expect(
      page.getByRole("button", { name: "Cayoo is the default project — clear it" }),
    ).toHaveAttribute("aria-pressed", "true");

    // Clicking the starred project clears it — New Session falls back to the first project.
    await page.getByRole("button", { name: "Cayoo is the default project — clear it" }).click();
...
test.describe("New-session Project → Folder (#448)", () => {
  ...
  test("the selected project prefills the folder; the picker overrides it", async ({ page }) => {
    await page.goto("/");
    await expect(page.getByLabel("Project", { exact: true })).toHaveValue("p-1"); // Cayoo is default
