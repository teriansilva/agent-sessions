...
test("streaming output does not yank a scrolled-up viewport back to the tail", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: /scroll to bottom/i })).toBeHidden();
});
...
test("the scroll-to-bottom button is available on desktop, hidden while on the tail", async ({
  ...
  test.skip(testInfo.project.name !== "desktop", "asserts the desktop (fine-pointer) gate");
  await page.goto("/s/claude/aaa");
  await expect(page.locator(".xterm-rows")).toContainText("LIVE tail", { timeout: 5000 });
  await page.waitForTimeout(1000);
