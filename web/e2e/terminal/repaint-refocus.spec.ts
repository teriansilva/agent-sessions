...
test("window refocus repaints a stale owner frame — no manual REPAINT", async ({ page }) => {
  await setupBench(page, { sessions: SESSIONS, history: { [KEY]: RICH }, wipeOnResizeChange: true });
  ...
  await expect(page.locator(".xterm-rows")).toContainText("LIVE (repainted)", { timeout: 8000 });
});

test("pageshow (bfcache restore) repaints a stale owner frame", async ({ page }) => {
  await setupBench(page, { sessions: SESSIONS, history: { [KEY]: RICH }, wipeOnResizeChange: true });
  await page.goto("/s/claude/aaaaaaaa-0000-4000-8000-00000000000a");
  await expect(page.locator(".xterm-rows")).toContainText("content line", { timeout: 15000 });
  await page.waitForTimeout(2500);
...
test("a read-only secondary viewer does NOT repaint on refocus", async ({ page }) => {
