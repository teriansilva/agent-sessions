...
function listing(path: string) {
  ...
  if (path === CWD) {
    return {
      ...
      entries: [
        ...
        {
          name: `${deep.replace(/\//g, "-")}.tsx`,
          path: `${CWD}/${deep.replace(/\//g, "-")}.tsx`,
          kind: "file",
          size: 2048,
          mtime: NOW,
        },
  ...
  if (path === "/home/u") {
    ...
    return {
      ...
      root: "/home/u",
      entries: [
        { name: "proj", path: CWD, kind: "dir", size: 4096, mtime: NOW },
      ],
      total: 1,
      complete: true,
      truncated: false,
...
test.describe("file panel", () => {
  test("opens from the pane head, expands a folder, opens a file, closes with Esc", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("[data-file-row][data-kind='dir']").first(),
    ).toBeVisible();
    await page.locator("[data-file-row]", { hasText: "src" }).first().click();
    await expect(
      page.locator("[data-file-row]", { hasText: "main.py" }),
    ).toBeVisible();
    ...
    await expect(
      page.locator(".xterm, [data-file-panel]").first(),
    ).toBeVisible();
  ...
  test("a symlink is display-only — it announces itself and does not open", async ({
    page,
  }) => {
    const trigger = await openSession(page);
    await trigger.click();
    const row = page.locator("[data-file-row][data-kind='link']");
    await expect(row).toBeVisible();
    // #792 made the row a CONTAINER with sibling controls, so the disabled state lives on the
    // control rather than on the row element — which is where it belongs anyway, since a <div>
    // cannot be disabled. The claim is unchanged; only the element carrying it moved.
    const link = row.locator("button").first();
  ...
  test("the page never scrolls sideways, even with a very long path", async ({
    page,
  }) => {
    const trigger = await openSession(page);
    await trigger.click();
  ...
  test("the viewer paints ABOVE the panel and the terminal's touch layer", async ({
    page,
  }) => {
...
test.describe("file panel — touch", () => {
  ...
  test("the sheet is reachable by TAP, above the terminal's capture overlay", async ({
    page,
  }) => {
    const trigger = await openSession(page);
    await trigger.tap();
    ...
    const hit = await page.evaluate(
      ({ x, y }) =>
        document.elementFromPoint(x, y)?.closest("[data-file-row]") !== null,
      { x: box!.x + box!.width / 2, y: box!.y + box!.height / 2 },
    );
  ...
  test("rows and controls are real 44px touch targets", async ({ page }) => {
    ...
    for (const sel of [
      ...
      expect(
        box!.height,
        `${sel} must be a real touch target`,
      ).toBeGreaterThanOrEqual(44);
    }
  });

  test("head chips do not overlap — a tap fires the action it looks like", async ({
    page,
  }) => {
    ...
    for (let i = 1; i < boxes.length; i++) {
      expect(
        boxes[i].left,
        "adjacent head chips must not overlap",
      ).toBeGreaterThanOrEqual(boxes[i - 1].right - 0.5);
    }
  });

  test("the sheet's scroll does not chain to the page behind it", async ({
    page,
  }) => {
    const trigger = await openSession(page);
    await trigger.tap();
...
test.describe("file panel — state and navigation", () => {
  test("Up stops at the server's root boundary instead of walking out of it", async ({
    page,
  }) => {
  ...
  test("closing the sheet hands focus back to the control that opened it", async ({
    page,
    isMobile,
  }) => {
  ...
  test("overflow arrow keys skip a disabled action and reach the ones behind it", async ({
    ...
    expect(focusedName, "focus must land on an ENABLED item").not.toBe(
      "Repaint screen",
    );
...
test.describe("file panel — contracts from review round 6", () => {
  test("an open panel does not carry A's root into B (no shared instance)", async ({
    page,
  }) => {
  ...
  test("the breadcrumb offers real ancestors, not just a reset", async ({
    page,
  }) => {
    const trigger = await openSession(page);
    await trigger.click();
    ...
    await expect(
      page.locator("[data-file-row]", { hasText: "proj" }),
    ).toBeVisible();
  });

  test("an unresolved cwd disables Files with a reason instead of removing it", async ({
    page,
  }) => {
    await mockApp(page);
  ...
  test("the mobile sheet contains Tab in both directions", async ({
    page,
    isMobile,
  }) => {
    test.skip(!isMobile, "sheet mode only");
    ...
    const inSheet = () =>
      page.evaluate(() =>
        Boolean(document.activeElement?.closest("[data-file-panel='sheet']")),
      );
...
test("arrow keys move focus between rows (#792 regression)", async ({
  page,
}) => {
  const trigger = await openSession(page);
  await trigger.click();
  ...
  // #792 turned the row into a container. Nothing caught that the arrow-key handler still
  // queried `[data-file-row]` and called `.focus()` on it — a <div> accepts the call and does
  // nothing, so navigation died silently while every existing test stayed green.
  const rows = page.locator("[data-row-main]");
