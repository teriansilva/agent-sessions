...
const ownsAttr = (page: import("@playwright/test").Page) =>
  page.evaluate(
    () =>
      document
        .querySelector("[data-app-owns-scroll]")
        ?.getAttribute("data-app-owns-scroll") ?? null,
  );
...
test("mouse-tracking session (claude) hides the useless native scrollbar", async ({
  page,
}, info) => {
  test.skip(
    info.project.name !== "desktop",
    "scrollbar chrome is a desktop concern",
  );
...
test("plain scrollback session (codex/antigravity) keeps its working scrollbar", async ({
  page,
}, info) => {
  test.skip(
    info.project.name !== "desktop",
