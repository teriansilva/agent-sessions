...
test("a project-entity group is labelled by the entity name with the path subtitle (#361)", () => {
  renderGroup({ project: "Side", kind: "project", cwd: "/home/u/one", cwdCount: 1, count: 1, collapsed: true });
...
test("an entity spanning several folders shows the folder count as subtitle", () => {
  renderGroup({ project: "Side", kind: "project", cwd: "/home/u/app", cwdCount: 3, count: 5, collapsed: true });
  expect(screen.getByText("Side")).toBeInTheDocument();
...
test("'Make this a project' POSTs {name: basename, folders: [cwd]} then refetches", async () => {
  ...
  vi.mocked(api.createProject).mockResolvedValue({
    id: "p-9", name: "one", color: "", folders: ["/home/u/one"], archived: false, created_at: 0,
  });
  renderGroup(
    { project: "one", kind: "folder", cwd: "/home/u/one", cwdCount: 1, count: 1, collapsed: true },
    refetch,
  );
  await user.click(screen.getByRole("button", { name: /make ~\/one a project/i }));
  expect(api.createProject).toHaveBeenCalledWith({ name: "one", folders: ["/home/u/one"] });
...
test("a 409 surfaces the server's detail inline and skips the refetch", async () => {
  const user = userEvent.setup();
  const refetch = vi.fn();
  vi.mocked(api.createProject).mockRejectedValue(new ApiError(409, "folder already owned by Side"));
  renderGroup(
    { project: "one", kind: "folder", cwd: "/home/u/one", cwdCount: 1, count: 1, collapsed: true },
    refetch,
  );
  await user.click(screen.getByRole("button", { name: /make ~\/one a project/i }));
  expect(await screen.findByText("folder already owned by Side")).toBeInTheDocument();
  expect(refetch).not.toHaveBeenCalled();
});

test("entity groups have no promote button (they already are projects)", () => {
  renderGroup({ project: "Side", kind: "project", cwd: "/home/u/app", cwdCount: 1, count: 1, collapsed: true });
