...
function letterFor(e: GitEntry): { ch: string; cls: string; label: string } {
  if (e.kind === "unmerged") return { ch: "U", cls: styles.gitConflict, label: "conflicted" };
  if (e.kind === "untracked") return { ch: "?", cls: styles.gitUntracked, label: "untracked" };
  ...
  if (ch === "R" || ch === "C") return { ch, cls: styles.gitMod, label: "renamed" };
  ...
  return (
    <div className={styles.body} data-git-tab="">
      <div className={styles.branchStrip}>
        ...
        <span className={styles.branchName} title={status.branch ?? "detached HEAD"}>
