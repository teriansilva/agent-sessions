...
test("changing a project's default folder patches default_folder (#448)", async () => {
  ...
  render(<ProjectsManagerCard />);
  await screen.findByText("Cayoo");
  await user.click(screen.getByRole("button", { name: "Change default folder for Cayoo" }));
  await user.click(screen.getByRole("button", { name: "stub-pick" }));
...
test("star: unstarred by default, and starring persists default_project_id", async () => {
  renderWithConfig();
  const star = await screen.findByRole("button", { name: "Make Cayoo the default project" });
  ...
  expect(
    await screen.findByRole("button", { name: "Cayoo is the default project — clear it" }),
...
test("star: seeds from config.default_project_id, and clicking the starred one clears it", async () => {
  ...
  expect(
    await screen.findByRole("button", { name: "Make Cayoo the default project" }),
...
test("star: exactly one project is starred at a time", async () => {
  ...
  await waitFor(() =>
    expect(screen.getByRole("button", { name: "Make Cayoo the default project" })).toHaveAttribute(
      "aria-pressed",
      "false",
    ),
  );
...
test("star: a failed save rolls back and surfaces an error", async () => {
  ...
  renderWithConfig();
  await userEvent.click(await screen.findByRole("button", { name: "Make Cayoo the default project" }));
