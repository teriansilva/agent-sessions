...
test("finals from an engine never identified as stacking are NEVER collapsed (#711 round 6)", () => {
  ...
  expect(assembleSpoken([fin("yes", 1000), fin("yes", 1499)], true)).toBe(
    "yes yes",
  );
  expect(assembleSpoken([fin("go", 1000), fin("go now", 1499)], true)).toBe(
    "go go now",
  );
  expect(assembleSpoken([fin("cat", 1000), fin("bat this", 1300)], true)).toBe(
    "cat bat this",
  );
});
...
test("a finalized empty entry is the fingerprint that arms collapsing (#711 finding 3)", () => {
  ...
  expect(
    assembleSpoken(
      [fin("", 0), fin("this", 300), fin("this is a test", 550)],
      true,
    ),
  ).toBe("this is a test");
  expect(
    assembleSpoken([fin("this", 300), fin("this is a test", 550)], true),
  ).toBe("this this is a test");
});
...
test("a repeated utterance spoken twice on the identified stacker survives (#711)", () => {
  expect(
    assembleSpoken([fin("", 0), fin("yes", 1000), fin("yes", 3000)], true),
  ).toBe("yes yes");
});
...
test("saying the same phrase three times slowly keeps all three, even on the stacker", () => {
  expect(
    assembleSpoken(
      [
        fin("", 0),
        fin("hello there", 1000),
        fin("hello there", 3000),
        fin("hello there", 5000),
      ],
      true,
    ),
  ...
  expect(
    assembleSpoken(
      [
        fin("", 0),
        fin("hey", 1000),
        fin("hey there", 1000 + SNAPSHOT_BURST_MS - 1),
      ],
      true,
    ),
  ).toBe("hey there");
  expect(
    assembleSpoken(
      [
        fin("", 0),
        fin("hey", 1000),
        fin("hey there", 1000 + SNAPSHOT_BURST_MS),
      ],
      true,
    ),
  ).toBe("hey hey there");
...
test("entries first exposed by the SAME event are preserved even on the identified stacker (#711 round 5)", () => {
  ...
  expect(
    assembleSpoken(
      [fin("", 500), fin("go", 1000, 7), fin("go now", 1000, 7)],
      true,
    ),
  ).toBe("go go now");
});
...
test("an interim-grown entry in the MIDDLE of the list never supersedes (#711/#749)", () => {
  ...
  expect(
    assembleSpoken(
      [fin("", 0), fin("go", 300), grown("go now", 500), fin("later", 4000)],
      true,
    ),
...
test("the LIVE entry may supersede on the identified stacker, interim-grown or not (#749)", () => {
  ...
  expect(
    assembleSpoken([fin("", 0), fin("go", 300), grown("go now", 500)], true),
  ).toBe("go now");
  // …and the gate still rules: no fingerprint, no collapse, however the entries are shaped.
  expect(assembleSpoken([fin("go", 300), grown("go now", 500)], true)).toBe(
    "go go now",
  );
});
...
test("an all-final lexical revision chain collapses to the corrected form (#711 round 4)", () => {
  ...
  expect(
    assembleSpoken(
      [fin("", 0), fin("dont", 300), fin("don't stop", 580)],
      true,
    ),
  ).toBe("don't stop");
});
...
test("a revision-shaped pair spoken seconds apart is two real utterances", () => {
  expect(
    assembleSpoken(
      [fin("", 0), fin("recognise", 1000), fin("recognize", 3500)],
      true,
    ),
  ).toBe("recognise recognize");
});

test("short words never alias each other as revisions — 'no' is not a draft of 'go'", () => {
  expect(
    assembleSpoken([fin("", 0), fin("no", 300), fin("go home", 600)], true),
  ).toBe("no go home");
});
...
test("a snapshot may extend its trailing word mid-recognition ('and' → 'android studio')", () => {
  expect(
    assembleSpoken(
      [fin("", 0), fin("and", 300), fin("android studio", 600)],
      true,
    ),
  ).toBe("android studio");
  ...
  expect(
    assembleSpoken(
      [fin("", 0), fin("and", 300), fin("android studio", 2300)],
      true,
    ),
  ).toBe("and android studio");
});

test("only the trailing word may be revised — interior words must match exactly", () => {
  expect(
    assembleSpoken(
      [fin("", 0), fin("and when", 300), fin("android when you", 600)],
      true,
    ),
  ).toBe("and when android when you");
});
...
test("punctuation-only finalized segments are preserved, not dropped (#711 finding 2)", () => {
  expect(
    assembleSpoken([fin("", 0), fin("hello", 300), fin(".", 500)], true),
  ).toBe("hello .");
  expect(
    assembleSpoken([fin("", 0), fin("use option", 300), fin("--", 500)], true),
  ).toBe("use option --");
});
...
test("a punctuation segment breaks a chain and is never swept up by a later prefix", () => {
  expect(
    assembleSpoken(
      [fin("", 0), fin("use", 300), fin("--", 500), fin("use option now", 700)],
      true,
    ),
...
test("a CJK snapshot chain on the identified stacker collapses by codepoint prefix (#711)", () => {
  expect(
    assembleSpoken(
      [fin("", 0), fin("你", 300), fin("你好", 600), fin("你好世界", 900)],
      false,
    ),
  ).toBe("你好世界");
});
...
test("CJK utterances spoken separately are preserved", () => {
  expect(
    assembleSpoken(
      [fin("", 0), fin("你好", 1000), fin("你好世界", 3500)],
      false,
    ),
  ).toBe("你好 你好世界");
});
...
test("empty and whitespace-only entries carry no speech and are dropped from output", () => {
  expect(
    assembleSpoken([fin("", 0), fin("   ", 100), fin("okay", 200)], true),
  ).toBe("okay");
...
test("interior whitespace is normalized in every mode", () => {
  expect(assembleSpoken([fin("  deploy   the    build  ", 0)], true)).toBe(
    "deploy the build",
  );
  expect(assembleSpoken([fin("  deploy   the    build  ", 0)], false)).toBe(
    "deploy the build",
  );
});
