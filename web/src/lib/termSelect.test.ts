...
import { DRAG_SLOP_PX, decideMouseDown, exceededSlop, forceSelectModifier } from "./termSelect";
...
describe("decideMouseDown (#617)", () => {
  ...
  test("no mouse tracking: xterm selects natively — never force (#582)", () => {
    ...
    expect(decideMouseDown(down({ detail: 2 }), { mouseTracking: false })).toBe("native");
  });

  test("double / triple click force-selects at once — no drag is coming", () => {
    expect(decideMouseDown(down({ detail: 2 }), { mouseTracking: true })).toBe("force-select");
    expect(decideMouseDown(down({ detail: 3 }), { mouseTracking: true })).toBe("force-select");
  });

  test("our own synthetic twin / click replay passes straight through to xterm", () => {
    expect(decideMouseDown(down({ isTrusted: false }), { mouseTracking: true })).toBe("native");
  });

  test("non-left buttons are left alone (context menu, middle paste)", () => {
    expect(decideMouseDown(down({ button: 1 }), { mouseTracking: true })).toBe("native");
    expect(decideMouseDown(down({ button: 2 }), { mouseTracking: true })).toBe("native");
  });

  test("an explicitly modified press is the user's intent, not ours", () => {
    for (const mod of ["shiftKey", "ctrlKey", "altKey", "metaKey"] as const) {
      expect(decideMouseDown(down({ [mod]: true }), { mouseTracking: true })).toBe("native");
    }
  });
});
...
describe("exceededSlop", () => {
  test("a press that never moves is a click", () => {
    ...
    expect(exceededSlop(100, 100, 100 + DRAG_SLOP_PX, 100 + DRAG_SLOP_PX)).toBe(false);
  });
...
describe("forceSelectModifier", () => {
  ...
  test("Shift off macOS, Alt on macOS — a Shift-only twin never selected on a Mac", () => {
    expect(forceSelectModifier(false)).toEqual({ shiftKey: true, altKey: false });
    expect(forceSelectModifier(true)).toEqual({ shiftKey: false, altKey: true });
  });
});
